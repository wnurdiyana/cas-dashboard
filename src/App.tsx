      ...
      <nav className="bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${view === 'member' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Member View
          </button>
          <button
            onClick={() => setView('admin')}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${view === 'admin' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Admin View
          </button>
        </div >
